...
router.post("/new_insight", async (req, res) => {
  try {
    ...
    const user = await User.findById(userId, { category_Id: 1 }).lean();
    ...
    payload.category_Id = user.category_Id;
...
// GET DASHBOARD LATEST INSIGHTS
router.get("/counts", check_Active_Subscription, async (req, res) => {
  try {
    const userId = req.user._id;
    const user = await User.findById(userId, { category_Id: 1 }).lean();
    ...
    const categories = await Insight.distinct("insightSubCategory");

    const categoryCounts = {};

    for (const category of categories) {
      const count = await Insight.countDocuments({
        insightSubCategory: category,
      });
